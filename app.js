...
app.get("/:word", (req, res) => {
  ...
  if (!utils.isValidWord(word)) {
    return res.status(400).json({ error: "Invalid input. The input must contain only alphabetic characters and be at least 2 characters long." });
  }
...
app.post("/add", (req, res) => {
  ...
  if (!fin && !eng) {
    return res.status(400).json({ error: "Both 'fin' and 'eng' fields are required." });
  } else if (!fin) {
    return res.status(400).json({ error: "'fin' field is required." });
  } else if (!eng) {
    return res.status(400).json({ error: "'eng' field is required." });
  } else if (!utils.isValidWord(fin)) {
    return res.status(400).json({ error: "Invalid 'fin' field. It must contain only alphabetic characters and be at least 2 characters long." });
  } else if (!utils.isValidWord(eng)) {
    return res.status(400).json({ error: "Invalid 'eng' field. It must contain only alphabetic characters and be at least 2 characters long." });
  }
